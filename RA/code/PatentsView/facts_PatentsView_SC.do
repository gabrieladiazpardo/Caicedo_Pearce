...
*Only keep patents produced in teams
keep if tsize>1
...
*** Survive for each year by sector 
...
*Consider survival of only teams tsize>1
gen survive2=survive if tsize>1
drop survive
rename survive2 survive


collapse (mean) tsize m_ttb cit3 f_cit3 survive frac_rep_coinv, by(appyear group)
...
local pl_str_fr_rcoinv  `pl_str_fr_rcoinv' (connected  frac_rep_coinv appyear if group==`ig')
...
if $save_fig ==1 {
graph export "$figfolder/tsize$gsave_str.pdf", replace as(pdf)
}
...
if $save_fig ==1 {
graph export "$figfolder/m_ttb$gsave_str.pdf", replace as(pdf)
}
...
if $save_fig ==1 {
graph export "$figfolder/survive$gsave_str.pdf", replace as(pdf)
}
...
if $save_fig ==1 {
graph export "$figfolder/cit3$gsave_str.pdf", replace as(pdf)
}
...
if $save_fig ==1 {
graph export "$figfolder/f_cit3$gsave_str.pdf", replace as(pdf)
}

*/

*Fraction of Repeated Coinventors
twoway `pl_str_fr_rcoinv' , ///
graphregion(color(white)) bgcolor(white) xtitle("Application Year",height(8) size($size_font)) xlabel($iyear(5)$fyear, labsize($size_font) ) ylabel(, nogrid labsize($size_font)) ////
ytitle("Fraction of Repeated Coinventors ",height(8) size($size_font))  xline(1995, lcolor(red) lpattern(dash)) legend(region(lcolor(white)) `lab_str' cols(4) )

if $save_fig ==1 {
graph export "$figfolder/frac_rep_coinv$gsave_str.pdf", replace as(pdf)
}
...
*******************************************
* Team Dynamics: Repeating Coinventors
*******************************************
{
...
*keep only necesary variables
keep  patent date appyear ipc3 type_asg tsize pr_alo_coinv frac_rep_coinv mpat_frac_rep_coinv mrpat_frac_rep_coinv pr_all_coinv pr_rpat
...
if $save_fig ==1 {
graph export "$figfolder/mpat_frac_rep_coinv$gsave_str.pdf", replace as(pdf)
}
...
if $save_fig ==1 {
graph export "$figfolder/mrpat_frac_rep_coinv$gsave_str.pdf", replace as(pdf)
}
...
*identifying dummies
bys appyear (ipc1): gen dyr_ipc1=1 if ipc1[_n-1]~=ipc1[_n]
bys appyear (tsize2): gen dyr_tsize=1 if tsize2[_n-1]~=tsize2[_n]

bys appyear ipc1: egen frac_rep_coinv_ipc1=mean(frac_rep_coinv)
bys appyear tsize2: egen frac_rep_coinv_tsize=mean(frac_rep_coinv)



keep if dyr_ipc1==1 | dyr_tsize==1
...
foreach l of local levels{
local pl_ipc1  `pl_ipc1' (connected  frac_rep_coinv_ipc1 appyear if ipc1=="`l'")
local lab_str  `lab_str' label( `i' "`l'") 
local i=`i'+1
}
...
twoway  `pl_ipc1' , ///
graphregion(color(white)) bgcolor(white) xtitle("Application Year",height(8) size($size_font)) xlabel($iyear(5)$fyear, labsize($size_font)) ylabel(, nogrid labsize($size_font)) ////
ytitle("Frac of Repeated Coinventors ",height(8) size($size_font)) legend(region(lcolor(white)) `lab_str' cols(4) ) xline(1995, lcolor(red) lpattern(dash))

if $save_fig==1 {
graph export "$figfolder/frac_rep_coinv_ipc1$gsave_str.pdf", replace as(pdf)
}

*By team size

twoway (connected  frac_rep_coinv_tsize appyear if tsize2==2, lwidth(medthick)) (connected  frac_rep_coinv_tsize appyear if tsize2==3, lwidth(medthick)) ///
(connected  frac_rep_coinv_tsize appyear if tsize2==4, lwidth(medthick)) (connected  frac_rep_coinv_tsize appyear if tsize2==5, lwidth(medthick)), ///
graphregion(color(white)) bgcolor(white) xtitle("Application Year",height(8) size($size_font)) xlabel($iyear(5)$fyear, labsize($size_font)) ylabel(, nogrid labsize($size_font)) ////
ytitle("Frac of Repeated Coinventors ",height(8) size($size_font)) xline(1995, lcolor(red) lpattern(dash)) ///
legend(region(lcolor(white))  label( 1 "2-person") label( 2 "3-person") label( 3 "4-person") label( 4 "5-person or more") size(med) ) 

if $save_fig==1 {
graph export "$figfolder/frac_rep_coinv_ts$gsave_str.pdf", replace as(pdf)
}
...
*********************************
* Dec 14 : New measures of interactions


** Number of distinct Coinventors

use "$datasets/survival_full_PV.dta", clear
