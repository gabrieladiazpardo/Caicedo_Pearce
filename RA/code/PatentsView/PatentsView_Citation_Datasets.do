**************************************************
*		Citation Datasets Input: PatentViews
*
*
**************************************************
...
*1) import data of the patent citated by each citing patent. Merges information about the citing patent (app year and grant date). Creates citations_by_yr_all
...
merge m:1 patent using "$datasets/tmp_first_assignee_uspto.dta", gen(m2)
...
We are only dropping those patents in Patent File that aren't "citing"= in other words, citing=.
*/
*10,169,430 observations Not Merged. Citing patents that will not have info. on assg
...
***28,156,158 Not Merged cited patents on assignee. We drop observations (firms) that have not been cited.
drop if _m==2
drop _m


*Generate Self Citations 
gen asg_self_cite=1 if cited_pdp==citing_pdp & cited_pdp~="" & cited_pdp~=""
	replace asg_self_cite=0 if asg_self_cite==.
	
*Gen Dummy for Identified Assignee

gen identif_asg=(cited_pdp~="" & cited_pdp~="")
...
save "$datasets/citations_by_yr_all.dta",replace
}

*2) create a panel dataset patent-year level cited with the number of forward citations over the years, max citations for each year, ipc1: Citation_event_time_ipc_all
...
drop if citingyear==.
save "$datasets/Citation_event_time_ipc_all.dta",replace
...
use "$datasets/tmp_patents_uspto_2.dta",clear

rename patent cited

*--Unir información panel patente-años y su appyear, grantyear, grantdate
merge 1:m cited using "$datasets/PatentsView/Citation_event_time_ipc_all.dta"
...
save "$datasets/PatentsView/Citation Profile1020_all.dta", replace
...
merge 1:m patent using "$datasets/PatentsView/citations_by_yr_all", gen(cit_merge)
*still a dyadics 
...
merge m:1 ipc1 using "$datasets/PatentsView/Citation Profile1020_all.dta"
keep if _merge==3

*--Edad de la patente (sólo se cogen patentes hasta 2020)
gen patent_age=2020-appyear
...
save "$datasets/PatentsView/life_cit_IPC1_all.dta",replace
...
save "$datasets/PatentsView/IPC3_dependence_long_agg_all.dta", replace
...
merge m:1 patent using "$datasets/PatentsView/life_cit_IPC1_all.dta"
drop _m
...
merge m:1 cited_sub using "$datasets/PatentsView/IPC3_dependence_long_agg_all.dta"
keep if _m==3
drop _m
...
keep patent categ1 inventor_id invh_cit exp_cit totpat
save "$datasets/PatentsView/citation_adjusted_all.dta",replace
