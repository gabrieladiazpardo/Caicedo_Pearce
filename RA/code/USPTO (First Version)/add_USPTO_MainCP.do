...
use "$datasets/citations_by_yr$save_str", replace
...
save "$datasets/f_cit$save_str", replace
...
merge m:1 patent using "$datasets/uspto_patent.dta"
...
merge m:1 patent using "$datasets/uspto_assignee_harmonized.dta"
keep if _m==3
drop _m
...
rename assignee_code citing_pdp
...
rename assignee_code cited_pdp
...
merge m:1 patent using "$datasets/uspto_patent.dta", gen(ay_merge2)
...
save "$datasets/b_cit$save_str.dta",replace
...
use "$datasets/main_CP$save_str.dta", clear

merge m:1 patent using "$datasets/f_cit$save_str"
drop _merge

merge m:1 patent using "$datasets/b_cit$save_str"
drop _merge
...
save "$datasets/f$save_str_main_CP.dta", replace


duplicates drop patent, force

keep patent gyear date appyear appdate ipc3 asgnum m_ttb tsize assignee f_cit cit3 cit5 exp_cit f_cit3 f_cit5 b_cit b_cit_asg_sc b_cit_ipc3_sc

save "$datasets/main_pat_lev_CP$save_str.dta", replace
