...
use "$datasets/uspto_patent.dta",clear
...
merge 1:m patent using "$datasets/citations_by_yr$save_str", gen(cit_merge)
*still a dyadics 
...
merge m:1 ipc1 using "$datasets/Citation Profile1020$save_str.dta"
keep if _merge==3
...
*****Meter Add_Main_CP - Meter back cit, lo que esta en add uspto
...
label var inventor_id "Inventor ID - original"
...
save "$datasets/main_CP$save_str.dta", replace
