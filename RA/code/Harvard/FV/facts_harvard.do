...
global figfolder="$main/RA/output/figfolder/harvard"

global save_str _Harvard
...
use "$datasets/main_pat_lev_CP$save_str.dta", clear
...
egen usptoclass=group(uspto_class)


collapse (count) usptoclass, by(appyear)


twoway (connected  usptoclass appyear, lcolor(navy) lwidth(medthick)) , ///
graphregion(color(white)) bgcolor(white) xtitle("Application Year",height(8) size($size_font)) xlabel($iyear(5)$fyear, labsize($size_font)) ylabel(, nogrid labsize($size_font)) ////
...
graphregion(color(white)) bgcolor(white) xtitle("Application Year",height(8) size($size_font)) xlabel($iyear(5)$fyear, labsize($size_font)) ylabel(80(10)130, nogrid labsize($size_font)) ////
...
*---------------------------
{
use "$datasets/f_harvard_main_cp.dta", clear
...
twoway (connected  dasg appyear, lcolor(navy) lwidth(medthick)) , ///
graphregion(color(white)) bgcolor(white) xtitle("Application Year",height(8) size($size_font)) xlabel($iyear(5)$fyear_pl, labsize($size_font)) ylabel(0(0.2)1, nogrid labsize($size_font)) ////
ytitle("Fraction of Identified Assignees",height(8) size($size_font)) xline(1995, lcolor(red) lpattern(dash))
...
collapse (mean) tsize m_ttb cit3 f_cit3 , by(appyear group)
