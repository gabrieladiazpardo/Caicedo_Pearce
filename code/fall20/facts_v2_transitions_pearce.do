...
merge m:1 patent using "/Users/JeremyPearce/Dropbox/Caicedo_Pearce/data/wrkdata/basicbib14_format_ipc.dta", keepusing(assignee pdpass)
keep if _m==3
gen ipc1=substr(ipc3,1,1)

egen teamgroup1=group(categ1 asgnum)
egen teamgroup2=group(categ1 assignee)
egen teamgroup3=group(categ1 pdpass)

egen g_ipc=group(ipc3)
xtile p10=patnumpp1, nq(10)
xtile t10=tsize, nq(10)
xtset categ1 patnumpp1
gen repeat_group1=.
replace repeat_group1=0 if teamgroup1!=L.teamgroup1 & L.teamgroup1~=.
replace repeat_group1=1 if teamgroup1==L.teamgroup1 & L.teamgroup1~=.

gen repeat_group2=.
replace repeat_group2=0 if teamgroup2~=L.teamgroup2 & L.teamgroup2~=.
replace repeat_group2=1 if teamgroup2==L.teamgroup2 & L.teamgroup2~=.

gen repeat_group3=.
replace repeat_group3=0 if teamgroup3~=L.teamgroup3 & L.teamgroup3~=.
replace repeat_group3=1 if teamgroup3==L.teamgroup3 & L.teamgroup3~=.

reghdfe repeat_group1 i.p10 i.t10, absorb(FE=i.g_ipc)
predict pred, xbd
gen adj_repeat1=repeat_group1-pred
drop FE
reghdfe repeat_group2 i.p10 i.t10, absorb(FE=i.g_ipc)
predict pred1, xbd
gen adj_repeat2=repeat_group2-pred


preserve
keep if assignee~=. & assignee~=0
collapse adj_repeat2 repeat_group2, by(appyear)
twoway connected adj_repeat appyear if appyear>=1980 & appyear<=2010, graphregion(color(white)) ytitle("Residual Probability Repeat Firm")
graph export "$figfolder/pearce/adj_repeat_indfirm_bb2.pdf",replace
twoway connected repeat_group appyear if appyear>=1980 & appyear<=2010, graphregion(color(white)) ytitle("Probability Repeat Firm")
graph export "$figfolder/pearce/noadj_repeat_indfirm_bb2.pdf",replace
restore


preserve
keep if asgnum~=""
collapse adj_repeat repeat_group, by(appyear)
twoway connected adj_repeat appyear if appyear>=1980 & appyear<=2010, graphregion(color(white)) ytitle("Residual Probability Repeat Firm")
graph export "$figfolder/pearce/adj_repeat_indfirm.pdf",replace
twoway connected repeat_group appyear if appyear>=1980 & appyear<=2010, graphregion(color(white)) ytitle("Probability Repeat Firm")
graph export "$figfolder/pearce/noadj_repeat_indfirm.pdf",replace
restore


bys asgnum: gen count=_n
...
collapse adj_repeat2 repeat_group2, by(patent assignee appyear)



bys assignee: gen count=_n
bys appyear: egen n10=xtile(count), nq(10)
collapse adj_repeat repeat_group, by(n10 appyear)
keep if appyear<=2008

#delimit ;
twoway (connected repeat_group appyear if appyear>=1980 & appyear<=2010 & n10==1)
(connected repeat_group appyear if appyear>=1980 & appyear<=2010 & n10==2)
(connected repeat_group appyear if appyear>=1980 & appyear<=2010 & n10==3)
(connected repeat_group appyear if appyear>=1980 & appyear<=2010 & n10==4)
(connected repeat_group appyear if appyear>=1980 & appyear<=2010 & n10==5)
(connected repeat_group appyear if appyear>=1980 & appyear<=2010 & n10==6)
(connected repeat_group appyear if appyear>=1980 & appyear<=2010 & n10==7)
(connected repeat_group appyear if appyear>=1980 & appyear<=2010 & n10==8)
(connected repeat_group appyear if appyear>=1980 & appyear<=2010 & n10==9)
(connected repeat_group appyear if appyear>=1980 & appyear<=2010 & n10==10), 
graphregion(color(white)) ytitle("Residual Probability Repeat Firm")
...
graph export "$figfolder/pearce/adj_repeat_indfirm_byfirmsize_bb.pdf", replace;
restore
...
bys assignee: gen count=_n
bys appyear: egen n10=xtile(count), nq(5)
collapse adj_repeat repeat_group, by(n10 appyear)
keep if appyear<=2008
...
(connected repeat_group appyear if appyear>=1980 & appyear<=2010 & n10==5), 
graphregion(color(white)) ytitle("Residual Probability Repeat Firm")
legend(order(1 "1st Quintile" 2 "2nd Quintile" 3 "3rd Quintile" 4 "4th Quintile" 5 "5th Quintile"));
graph export "$figfolder/pearce/adj_repeat_indfirm_byfirmsize5_bb.pdf", replace;
restore
...
collapse adj_repeat2 repeat_group2 patnumpp1, by(patent assignee appyear)
...
bys appyear: egen n10=xtile(count), nq(10)
gen lnc=ln(count)
gen lnp=ln(patnumpp1)
gen cor1=.
gen cor2=.
forvalues i=1980/2008{

corr lnc adj_repeat2 if appyear==`i'
...
collapse cor1 cor2, by(appyear)
twoway (connected cor1 appyear if appyear>=1980 & appyear<=2008), graphregion(color(white)) ytitle("Corr Repeated Inventor & Firm Size")
graph export "$figfolder/pearce/cor_repeat_firmsize.pdf", replace


twoway (connected cor2 appyear if appyear>=1980 & appyear<=2008), graphregion(color(white)) ytitle("Corr Inventor Age & Firm Size")
graph export "$figfolder/pearce/cor_inventorage_firmsize_overtime.pdf", replace
restore



***SPIN OFF RATE:
bys assignee (date): gen count=_n
xtset categ1 patnumpp1
gen prev_firm=L.count
gen move_change=count-prev_firm
replace move_change=. if L.assignee==. | L.assignee==0 | assignee==. | assignee==0
...
preserve
drop if categ1==.
keep if L.assignee~=assignee
***here we will adjust for team suze
collapse move_change, by(appyear tsize)
keep if appyear<=2008
rename tsize n10
*replace move_change=ln(move_change)
#delimit ;
twoway (connected move_change appyear if appyear>=1980 & appyear<=2010 & n10==1)
(connected move_change appyear if appyear>=1980 & appyear<=2010 & n10==2)
(connected move_change appyear if appyear>=1980 & appyear<=2010 & n10==3)
(connected move_change appyear if appyear>=1980 & appyear<=2010 & n10==4)
(connected move_change appyear if appyear>=1980 & appyear<=2010 & n10==5), 
graphregion(color(white)) ytitle("Diff in Firm Size")
legend(order(1 "Alone" 2 "Team Size=2" 3 "Team Size=3" 4 "Team Size=4" 5 "Team Size=5"));
graph export "$figfolder/pearce/move_change_bytsize_overtime.pdf", replace;
restore



preserve
drop prev_firm move_change count
drop if categ1==. | assignee==0 | assignee==.
bys assignee appyear (date): gen count=_n
xtset categ1 patnumpp1
gen prev_firm=L.count
gen move_change=count-prev_firm
keep if L.assignee~=assignee
...
graphregion(color(white)) ytitle("Diff in Firm Size")
legend(order(1 "Alone" 2 "Team Size=2" 3 "Team Size=3" 4 "Team Size=4" 5 "Team Size=5"));
graph export "$figfolder/pearce/move_change_bytsize_overtimeyear.pdf", replace;
restore



**spin off rate
replace count=. if assignee==0 | assignee==.
bys appyear: egen n10=xtile(count), nq(10)
xtset categ1 patnumpp1
gen spinoff=.
replace spinoff=0 if (n10==9 | n10==10) & F.patnumpp1~=. & F.assignee==assignee
replace spinoff=1 if  (n10==9 | n10==10) & F.patnumpp1~=. & (F.n10==1 | F.assignee==0)

drop if assignee==0
sum spinoff if appyear==2009

preserve
collapse spinoff, by(appyear tsize)
keep if appyear>=1980 & appyear<=2010
rename spinoff move_change
rename tsize n10
#delimit ;
twoway (connected move_change appyear if appyear>=1980 & appyear<=2010 & n10==1)
(connected move_change appyear if appyear>=1980 & appyear<=2010 & n10==2)
...
(connected move_change appyear if appyear>=1980 & appyear<=2010 & n10==4)
(connected move_change appyear if appyear>=1980 & appyear<=2010 & n10==5), 
graphregion(color(white)) ytitle("Spinoff Rate")
legend(order(1 "Alone" 2 "Team Size=2" 3 "Team Size=3" 4 "Team Size=4" 5 "Team Size=5"));
graph export "$figfolder/pearce/spinoffrate_bytsize_overtimeyear.pdf", replace;
restore;


preserve
gen ftsize=F.tsize
collapse spinoff, by(appyear ftsize)
keep if appyear>=1980 & appyear<=2008
rename spinoff move_change
rename ftsize n10
#delimit ;
twoway (connected move_change appyear if appyear>=1980 & appyear<=2010 & n10==1)
...
graphregion(color(white)) ytitle("Spinoff Rate")
legend(order(1 "Alone" 2 "Team Size=2" 3 "Team Size=3" 4 "Team Size=4" 5 "Team Size=5"));
graph export "$figfolder/pearce/spinoffrate_by_ftsize_overtimeyear.pdf", replace;
restore;
...
bys assignee (date): gen count2=_n
bys appyear: egen n5=xtile(count2), nq(5)
collapse tsize, by(appyear n5)
keep if appyear>=1980 & appyear<=2008
rename tsize move_change
rename n5 n10
#delimit ;
twoway (connected move_change appyear if appyear>=1980 & appyear<=2010 & n10==1)
(connected move_change appyear if appyear>=1980 & appyear<=2010 & n10==2)
...
(connected move_change appyear if appyear>=1980 & appyear<=2010 & n10==4)
(connected move_change appyear if appyear>=1980 & appyear<=2010 & n10==5), 
graphregion(color(white)) ytitle("Team Size by Firm Size")
legend(order(1 "1st Quintile" 2 "2nd Quintile" 3 "3rd Quintile" 4 "4th Quintile" 5 "5th Quintile"));
graph export "$figfolder/pearce/tsize_by_firmsize_overtime.pdf", replace;
restore;
...
bys assignee (date): gen count2=_n
bys appyear: egen n5=xtile(count2), nq(5)
collapse survive, by(appyear n5)
keep if appyear>=1980 & appyear<=2008
rename survive move_change
rename n5 n10
#delimit ;
twoway (connected move_change appyear if appyear>=1980 & appyear<=2010 & n10==1)
(connected move_change appyear if appyear>=1980 & appyear<=2010 & n10==2)
...
(connected move_change appyear if appyear>=1980 & appyear<=2010 & n10==4)
(connected move_change appyear if appyear>=1980 & appyear<=2010 & n10==5), 
graphregion(color(white)) ytitle("Team Survival by Firm Size")
legend(order(1 "1st Quintile" 2 "2nd Quintile" 3 "3rd Quintile" 4 "4th Quintile" 5 "5th Quintile"));
graph export "$figfolder/pearce/teamsurvive_by_firmsize_overtime.pdf", replace;
restore;
...
gen moved_firm=.
xtset categ1 patnumpp1
replace moved_firm=0 if L.assignee~=.
replace moved_firm=1 if L.assignee~=assignee & L.assignee~=.
bys assignee (date): gen count2=_n
bys appyear: egen n5=xtile(count2), nq(5)
collapse moved_firm, by(appyear n5)
rename moved_firm move_change
keep if appyear>=1980 & appyear<=2008
rename n5 n10
#delimit ;
twoway (connected move_change appyear if appyear>=1980 & appyear<=2005 & n10==1)
(connected move_change appyear if appyear>=1980 & appyear<=2005 & n10==2)
(connected move_change appyear if appyear>=1980 & appyear<=2005 & n10==3)
(connected move_change appyear if appyear>=1980 & appyear<=2005 & n10==4)
(connected move_change appyear if appyear>=1980 & appyear<=2005 & n10==5), 
graphregion(color(white)) ytitle("Move Across Firms by Firm Size")
legend(order(1 "1st Quintile" 2 "2nd Quintile" 3 "3rd Quintile" 4 "4th Quintile" 5 "5th Quintile"));
graph export "$figfolder/pearce/movefirms_by_firmsize_overtime.pdf", replace;
restore;
...
replace moved_firm=1 if L.assignee~=assignee & L.assignee~=.
bys assignee (date): gen count2=_n
bys appyear: egen n5=xtile(count2), nq(10)
xtset categ1 patnumpp1
gen firm_changesize=n5-L.n5
collapse firm_changesize, by(appyear tsize)
keep if tsize<=5
rename tsize n5
rename firm_changesize move_change
keep if appyear>=1980 & appyear<=2008
rename n5 n10
#delimit ;
twoway (connected move_change appyear if appyear>=1980 & appyear<=2005 & n10==1)
...
(connected move_change appyear if appyear>=1980 & appyear<=2005 & n10==4)
(connected move_change appyear if appyear>=1980 & appyear<=2005 & n10==5), 
graphregion(color(white)) ytitle("Move Across Firm Decile by Team Size")
legend(order(1 "Team Size 1" 2 "2" 3 "3" 4 "4" 5 "5"));
graph export "$figfolder/pearce/movefirmsdec_by_tsize_overtime.pdf", replace;
restore;


twoway connected spinoff appyear
