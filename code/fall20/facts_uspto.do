...
{
**Number of patents
use "$datasets/uspto_patent_lev_dataset.dta", clear

drop if ipc3=="" | rare_ipc3==1
...
egen  HHI_ipc1=sum(dipc1_year*sqs_ipc1), by(appyear)
egen  HHI_ipc3=sum(dipc3_year*sqs_ipc3), by(appyear)

encode ipc3, gen(ipc3n)

*In thousands
local lvar tot_pat n_ipc1 n_ipc3

foreach ivar of local lvar{
	replace `ivar'=`ivar'/1000
}

**Number of patents

*Total number of patents
twoway (connected  tot_pat appyear, lcolor(navy) lwidth(medthick)) if dyear==1 ,  ///
...
ytitle("Number of Patents (thousands)",height(8) size($size_font)) xline(1995, lcolor(red) lpattern(dash))

if $save_fig==1 {
graph export "$figfolder/tot_pat$save_str.pdf", replace as(pdf)
}
...
twoway  `pl_n_str' if dipc1_year==1, ///
graphregion(color(white)) bgcolor(white) xtitle("Application Year",height(8) size($size_font)) xlabel($iyear(5)$fyear, labsize($size_font)) ylabel(, nogrid labsize($size_font)) ////
ytitle("Number of Patents (thousands)",height(8) size($size_font)) legend(region(lcolor(white)) `lab_str' cols(4) ) xline(1995, lcolor(red) lpattern(dash))

if $save_fig==1 {
graph export "$figfolder/n_ipc1$save_str.pdf", replace as(pdf)
}

twoway  `pl_s_str' if dipc1_year==1 , ///
graphregion(color(white)) bgcolor(white) xtitle("Application Year",height(8) size($size_font)) xlabel($iyear(5)$fyear, labsize($size_font)) ylabel(, nogrid labsize($size_font)) ////
...
if $save_fig==1 {
graph export "$figfolder/s_ipc1$save_str.pdf", replace as(pdf)
}

**Herfindahl-Hirschman Index

*IPC1
twoway (connected  HHI_ipc1 appyear, lcolor(navy) lwidth(medthick)) if dipc1_year==1 , ///
graphregion(color(white)) bgcolor(white) xtitle("Application Year",height(8) size($size_font)) xlabel($iyear(5)$fyear, labsize($size_font)) ylabel(, nogrid labsize($size_font)) ////
ytitle("Concentration (HHI) IPC1",height(8) size($size_font)) xline(1995, lcolor(red) lpattern(dash))

if $save_fig==1 {
graph export "$figfolder/HHI_ipc1$save_str.pdf", replace as(pdf)
}

*IPC3
twoway (connected  HHI_ipc3 appyear, lcolor(navy) lwidth(medthick)) , ///
graphregion(color(white)) bgcolor(white) xtitle("Application Year",height(8) size($size_font)) xlabel($iyear(5)$fyear, labsize($size_font)) ylabel(, nogrid labsize($size_font)) ////
ytitle("Concentration (HHI) IPC3",height(8) size($size_font)) xline(1995, lcolor(red) lpattern(dash))

if $save_fig==1 {
graph export "$figfolder/HHI_ipc3$save_str.pdf", replace as(pdf)
}
...
keep inventor_id appyear

bys appyear: gen Ninv=_N 

bys appyear: gen dyear=1 if appyear[_n]!=appyear[_n-1]


*In thousands
replace Ninv=Ninv/1000

twoway (connected  Ninv appyear, lcolor(navy) lwidth(medthick)) if dyear==1, ///
graphregion(color(white)) bgcolor(white) xtitle("Application Year",height(8) size($size_font)) xlabel($iyear(5)$fyear, labsize($size_font)) ylabel(, nogrid labsize($size_font)) ////
ytitle("Number of Inventors (thousands)",height(8) size($size_font)) xline(1995, lcolor(red) lpattern(dash))

if $save_fig==1 {
graph export "$figfolder/inventors$save_str.pdf", replace as(pdf)
}
}
*--------------------------------
...
drop if inventor_id=="" | rare_ipc3==1
keep if appyear>=$iyear & appyear<=$fyear

keep inventor_id appyear ipc3

gen ipc1=substr(ipc3,1,1)

bys inventor_id : gen npat=_N
bys inventor_id ipc1 : gen npat_ipc1=_N
bys inventor_id ipc3 :  gen npat_ipc3=_N


*Running number of patents
sort inventor_id appyear ipc3

bys inventor_id ipc3 (appyear ipc3):  gen rpat_ipc3=_n
bys inventor_id (appyear ipc3 rpat_ipc3): gen rpat=_n
bys inventor_id ipc1 (appyear ipc3 rpat_ipc3): gen rpat_ipc1=_n


sort inventor_id appyear ipc3 rpat_ipc3

*count number of different classes by inventor
bys inventor_id (ipc3 appyear rpat_ipc3) : gen dipc3=1 if ipc3[_n]!=ipc3[_n-1]
bys inventor_id : egen nipc3=total(dipc3)
bys inventor_id (appyear ipc3 rpat_ipc3)  : gen ripc3=sum(dipc3)

bys inventor_id (ipc3 appyear rpat_ipc3) : gen dipc1=1 if ipc1[_n]!=ipc1[_n-1]
bys inventor_id : egen nipc1=total(dipc1)
bys inventor_id (appyear ipc3 rpat_ipc3)  : gen ripc1=sum(dipc1)
...
bys inventor_id (appyear) : egen iyear=min(appyear)
bys inventor_id (appyear) : egen fyear=max(appyear)
...
gen gr_10yr=1 if iyear>=1975 & iyear<=1984
	...
	replace gr_10yr=4 if iyear>=2004 & iyear<=2014
...
if $save_fig==1 {
graph export "$figfolder/npat_hist$save_str$save_str_solo.pdf", replace as(pdf)
}
...
if $save_fig==1 {
graph export "$figfolder/ripc3$save_str$save_str_solo.pdf", replace as(pdf)
}
...
if $save_fig==1 {
graph export "$figfolder/ripc1_pat$save_str$save_str_solo.pdf", replace as(pdf)
}
...
if $save_fig==1 {
graph export "$figfolder/ripc3_pat$save_str$save_str_solo.pdf", replace as(pdf)
}
...
(line  ripc3 rpat if gr_10yr==3, lcolor(dkgreen) lwidth(medthick) lpattern("-..")) (line  ripc3 rpat if gr_10yr==4, lcolor(orange) lwidth(medthick) lpattern("--.")) ///
if rpat<=`maxrpat', ///
...
if $save_fig==1 {
graph export "$figfolder/ripc3_rpat_10yr$save_str$save_str_solo.pdf", replace as(pdf)
}
...
(line  ripc1 rpat if gr_10yr==3, lcolor(dkgreen) lwidth(medthick) lpattern("-..")) (line  ripc1 rpat if gr_10yr==4, lcolor(orange) lwidth(medthick) lpattern("--.")) ///
 if rpat<=`maxrpat', ///
graphregion(color(white)) bgcolor(white) xtitle("Number of Patents",height(8) size($size_font)) xlabel(0(`dpat')`maxrpat', labsize($size_font)) ylabel(, nogrid labsize($size_font)) ///
...
if $save_fig==1 {
graph export "$figfolder/ripc1_rpat_10yr$save_str$save_str_solo.pdf", replace as(pdf)
}
...
(line ripc3 rpat if gr_5yr==7,  lwidth(medthick) lpattern("---.."))  (line  ripc3 rpat if gr_5yr==8,  lwidth(medthick) lpattern("-...")) if rpat<=`maxrpat', ///
graphregion(color(white)) bgcolor(white) xtitle("Number of Patents",height(8) size($size_font)) xlabel(0(`dpat')`maxrpat', labsize($size_font)) ylabel(, nogrid labsize($size_font)) ///
ytitle("Number of IPC3",height(8) size($size_font)) ///
...
label(7 "2005-2009") label(8 "2010-2014") )

if $save_fig==1 {
graph export "$figfolder/ripc3_rpat_5yr$save_str$save_str_solo.pdf", replace as(pdf)
}
...
(line ripc1 rpat if gr_5yr==7,  lwidth(medthick) lpattern("---.."))  (line  ripc1 rpat if gr_5yr==8,  lwidth(medthick) lpattern("-...")) if rpat<=`maxrpat', ///
graphregion(color(white)) bgcolor(white) xtitle("Number of Patents",height(8) size($size_font)) xlabel(0(`dpat')`maxrpat', labsize($size_font)) ylabel(, nogrid labsize($size_font)) ///
ytitle("Number of IPC1",height(8) size($size_font)) ///
legend(region(lcolor(white))  label(1 "1975-1979") label(2 "1980-1984" ) label(3 "1985-1989") label(4 "1990-1994") label(5 "1995-1999" ) label(6 "2000-2004") ///
label(7 "2005-2009") label(8 "2010-2014") )
...
if $save_fig==1 {
graph export "$figfolder/ripc1_rpat_5yr$save_str$save_str_solo.pdf", replace as(pdf)
}
...
collapse (mean) tsize m_ttb survive cit3 f_cit3 , by(appyear group)
...
if $save_fig ==1 {
graph export "$figfolder/cit3`lsave_str'.pdf", replace as(pdf)
}
...
if $save_fig ==1 {
graph export "$figfolder/f_cit3`lsave_str'.pdf", replace as(pdf)
}
...
use "$data/main_asg_lev_bp_CP$save_str.dta", clear
...
*1. Assignee mobility (do same thing for 3 or 5 year mobility)

*2. Age trends and dynamics of firms, look at literature

*3. Inventor trends and churning, how does assortative matching been behaving? Look at Ma paper
